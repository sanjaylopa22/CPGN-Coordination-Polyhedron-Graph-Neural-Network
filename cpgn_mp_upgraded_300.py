...
EPOCHS        = 300           # Full 300 epochs — no early stopping
BATCH_SIZE    = 64
WEIGHT_DECAY  = 1e-4   # increase from 1e-5 — reduces overfitting
LEARNING_RATE = 1e-4   # reduce from 3e-4 — slower but less overfitting
...
def train_model(train_ds, val_ds):
    ...
    # Cosine annealing over full 300 epochs — no early stopping
    ...
    print(f"  Epochs      : {EPOCHS} (full — no early stopping)")
    print(f"  Batch       : {BATCH_SIZE}  LR : {LEARNING_RATE}")
    print(f"  Loss        : MAE (primary Ef) + {LAMBDA_AUX}×(MAE Bg)")
    print(f"  Scheduler   : CosineAnnealingLR  T_max={EPOCHS}  eta_min=1e-6")
    print("  Checkpoint  : best val Ef MAE (saved throughout all epochs)")
    print(f"{'='*70}")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        # ── checkpoint on best val Ef MAE (runs every epoch, no stopping) ────
        if vm < best_val_mae - 1e-5:
            ...
            print(f"  [ckpt] ep={ep:>3}  new best val MAE = {best_val_mae:.4f}")

    # ── end of training: load best checkpoint for inference ──────────────────
    print(f"\n  Total : {time.time()-t0_total:.1f}s  |  "
          f"Best val Ef MAE = {best_val_mae:.4f} eV/atom  (ep saved to {CHECKPOINT})")
    if best_state:
...
def main():
    ...
    for f in [CHECKPOINT, OUTPUT_VAL_CSV, OUTPUT_CSV,
              "CPGN_MP_training_curve_300.png",
              "CPGN_MP_parity_plots_300.png",
              "CPGN_MP_error_hist_300.png",
              "CPGN_MP_benchmark_table_300.png"]:
