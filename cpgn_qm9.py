...
WEIGHT_DECAY  = 1e-5
PATIENCE      = 50
LAMBDA_AUX    = 0.1      # global auxiliary weight (kept for compatibility)

# ── Per-property auxiliary loss weights ──────────────────────────────────────
# Each auxiliary property gets its own weight so no single property starves.
# Order matches QM9_PROPS auxiliary list (HOMO excluded):
#   LUMO, Gap, ZPVE, mu, alpha, R2, U0, U298, H298, G298
LAMBDA_AUX_PER_PROP = {
    "LUMO" : 0.5,    # orbital energy — closely related to HOMO
    "Gap"  : 0.5,    # HOMO-LUMO gap — critical electronic property
    "ZPVE" : 0.3,    # vibrational energy
    "mu"   : 0.2,    # dipole moment — harder to learn
    "alpha": 0.2,    # polarisability — harder to learn
    "R2"   : 0.5,    # spatial extent — size-correlated, important
    "U0"   : 0.3,    # atomisation energy at 0 K
    "U298" : 0.3,    # atomisation energy at 298 K
    "H298" : 0.3,    # enthalpy
    "G298" : 0.3,    # Gibbs free energy
}
...
QM9_PROPS = [
    ...
    ("Gap",   "ε_gap",    "eV",      4,       "relu",      False),
    ("ZPVE",  "ZPVE",     "eV",      6,       "relu",      False),
    ("mu",    "μ",        "D",       0,       "relu",      False),
    ("alpha", "α",        "Bohr³",   1,       "relu",      False),
    ("R2",    "〈R²〉",   "Bohr²",   5,       "relu",      False),
...
class CPGN(nn.Module):
    ...
    def __init__(self):
        ...
        # ── Output heads — 2-layer MLP per property ──────────────────────────
        # 2-layer heads (Linear→SiLU→Linear) give each property
        # a dedicated non-linear transformation from the shared latent z.
        # This is critical for properties like R2, alpha, mu that have
        # complex non-linear relationships with molecular structure.
        H2 = PRED_DIM // 2   # 64

        # Primary: HOMO  (2-layer, no final activation — can be negative)
        self.head_primary = nn.Sequential(
            nn.Linear(PRED_DIM, H2), nn.SiLU(),
            nn.Linear(H2, 1)
        )

        # Auxiliary: one 2-layer head per remaining QM9 property
        aux_props = [(s, l, u, pyg, act, pri)
                     for (s, l, u, pyg, act, pri) in QM9_PROPS
                     if not pri]
        self.aux_heads = nn.ModuleList([
            nn.Sequential(nn.Linear(PRED_DIM, H2), nn.SiLU(), nn.Linear(H2, 1))
            for _ in aux_props
        ])
    ...
    def forward(self, batch):
        ...
        for head, act in zip(self.aux_heads, self._aux_acts):
            ...
            if act == "softplus":
                ...
                p = F.relu(p)          # harder non-negativity, avoids softplus saturation
            elif act == "sigmoid":
                p = torch.sigmoid(p)
            aux_preds.append(p)
...
# ============================================================
# Build per-property weight tensor once (order = LUMO,Gap,ZPVE,mu,alpha,R2,U0,U298,H298,G298)
_AUX_PROP_ORDER = ["LUMO","Gap","ZPVE","mu","alpha","R2","U0","U298","H298","G298"]
_AUX_WEIGHTS    = torch.tensor(
    [LAMBDA_AUX_PER_PROP[p] for p in _AUX_PROP_ORDER],
    dtype=torch.float32)

class CPGNLoss(nn.Module):
    """
    Per-property weighted MAE loss.
    Each auxiliary target has its own gradient weight so no property starves.
    Primary (HOMO) still receives weight 1.0.
    """
    def forward(self, preds, batch):
        loss_primary = F.l1_loss(preds["primary"], batch.y_primary.squeeze(-1))

        # Per-property MAE: (B, N_AUX) → (N_AUX,)
        per_prop_mae = (preds["aux"] - batch.y_aux).abs().mean(dim=0)

        # Weighted sum of per-property MAEs
        weights  = _AUX_WEIGHTS.to(per_prop_mae.device)
        loss_aux = (per_prop_mae * weights).sum()
...
def train_model(train_ds, val_ds):
    ...
    print(f"  Checkpoint    : best val mean MAE (HOMO+LUMO+Gap+ZPVE) → {CHECKPOINT}")
    print(f"{'='*70}")
